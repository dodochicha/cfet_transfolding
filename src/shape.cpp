...
std::vector<Shape*> finger_slot_configuration(int finger) {
    ...
    // int row = 2;
    int row = std::min(2, max_allowable_cell_height);  // finger slot has maximum height 2
    int column = finger;
...
void folding_shape_generation() {
    ...
    for (Transistor* p : pmos) {
        ...
        } else {  // p->num_finger <= n->num_finger
            for (auto p_shape : p->es) {
                for (auto n_shape : n->es) {
                    std::vector<std::vector<bool>> stack_feasible = stack_feasibility(n_shape, p_shape);
                    // std::cout << "big shape: " << std::endl;
                    // for (int i = 0; i < n_shape->config.size(); i++) {
                    //     for (int j = 0; j < n_shape->config[i].size(); j++) {
                    //         std::cout << n_shape->config[i][j];
                    //     }
                    //     std::cout << std::endl;
                    // }
                    // std::cout << "small shape: " << std::endl;
                    // for (int i = 0; i < p_shape->config.size(); i++) {
                    //     for (int j = 0; j < p_shape->config[i].size(); j++) {
                    //         std::cout << p_shape->config[i][j];
                    //     }
                    //     std::cout << std::endl;
                    // }
                    // std::cout << "stack_feasible: " << std::endl;
                    // for (int i = 0; i < stack_feasible.size(); i++) {
                    //     for (int j = 0; j < stack_feasible[i].size(); j++) {
                    //         std::cout << stack_feasible[i][j];
                    //     }
                    //     std::cout << std::endl;
                    // }
                    phi_merged[p].insert(std::make_pair(n_shape, std::vector<Lambda*>()));
...
std::vector<std::vector<bool>> stack_feasibility(Shape* big_shape, Shape* small_shape) {
    ...
    for (int x = 0; x < big_shape->config.size(); x++) {
        for (int y = 0; y < big_shape->config[x].size(); y++) {
