...
// bool CFET::diffusion_sharing_single_row(std::vector<std::vector<int>> config_left, std::vector<std::vector<int>> config_right, Transistor* tr_left,
//                                         Transistor* tr_right) {
...
//             (tr_left_right_signals[1] == tr_right_left_signals[1] || tr_left_right_signals[1] == nullptr || tr_right_left_signals[1] == nullptr));
// }
    ...
    for (int i = row; i < row + new_lamb->config_up.size(); i++) {
        ...
        if (i >= old_pshape->multirow_tr_shape_up.size()) {
            ...
            if (new_lamb->most_left_id[i - row] + col - 1 < old_pshape->multirow_tr_shape_up[i - row].size()) {
                up_old_signal = get_right_active(old_pshape->multirow_tr_permutation_up[i][new_lamb->most_left_id[i - row] + col - 1],
                                                 old_pshape->multirow_tr_shape_up[i][new_lamb->most_left_id[i - row] + col - 1]);
                down_old_signal = get_right_active(old_pshape->multirow_tr_permutation_down[i][new_lamb->most_left_id[i - row] + col - 1],
                                                   old_pshape->multirow_tr_shape_down[i][new_lamb->most_left_id[i - row] + col - 1]);
            } else {
