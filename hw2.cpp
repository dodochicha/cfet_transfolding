...
unsigned char* gathered_image;
unsigned char* raw_image;  // 1D image
unsigned char** image;     // 2D image

// save raw_image to PNG file
void write_png(const char* filename) {
    unsigned error = lodepng_encode32_file(filename, gathered_image, width, height);
...
int main(int argc, char** argv) {
    ...
    // Each process calculates the portion of the image it will render
    int rows_per_process = height / world_size;
    int start_row = world_rank * rows_per_process;
    int end_row = (world_rank + 1 == world_size) ? height : start_row + rows_per_process;

    //---create image
    //raw_image = new unsigned char[width * height * 4];
    //image = new unsigned char*[height];
    raw_image = new unsigned char[width * (end_row - start_row) * 4];
    image = new unsigned char*[end_row - start_row];
    ...
    for (int i = start_row; i < end_row; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            fcol *= 255.0;
            //image[index][4 * j + 0] = (unsigned char)fcol.r;  // r
            //image[index][4 * j + 1] = (unsigned char)fcol.g;  // g
            //image[index][4 * j + 2] = (unsigned char)fcol.b;  // b
            //image[index][4 * j + 3] = 255;                    // a
            
            raw_image[4 * (j + index * width) + 0] = (unsigned char)fcol.r;  // r
            raw_image[4 * (j + index * width) + 1] = (unsigned char)fcol.g;  // g
            raw_image[4 * (j + index * width) + 2] = (unsigned char)fcol.b;  // b
            raw_image[4 * (j + index * width) + 3] = 255;                    // a
            //std::cout << 4 * (j + index * width) + 0 << " " << fcol.r << std::endl;
            current_pixel++;
            // print progress
    ...
    unsigned char* gathered_image = nullptr;
    if (world_rank == 0) {
        //gathered_image = new unsigned char[width * height * 4]; // Full image
        gathered_image = (unsigned char*)malloc(width * height * 4 * sizeof(unsigned char));
    }
    int row_num = (world_rank + 1 == world_size) ? height - rows_per_process * (world_size - 1) : rows_per_process;
    std::cout << world_rank << " " << row_num * width * 4 << " " << width * 4 * world_rank * rows_per_process << " " << width * height * 4 << std::endl;
    
   // MPI_Gather(raw_image, row_num * width * 4, MPI_UNSIGNED_CHAR,
              // gathered_image + width * 4 * world_rank * rows_per_process, row_num * width * 4, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    MPI_Gather(raw_image, row_num * width * 4, MPI_UNSIGNED_CHAR,
               gathered_image, row_num * width * 4, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    //MPI_Gather(raw_image, 10, MPI_UNSIGNED_CHAR,
               //gathered_image, 10, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    std::cout << world_rank << " " << row_num * width * 4 << " " << width * 4 * world_rank * rows_per_process << " " << width * height * 4 << std::endl;
    //---saving image
    //write_png(argv[10]);
    // Root process saves the gathered image
    if (world_rank == 0) {
        write_png(argv[10]);
        delete[] gathered_image; // Free memory after saving
    }
    //---
